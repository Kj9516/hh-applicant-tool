...
| **apply-from-file**       | Откликнуться на вакансии из CSV/TSV файла, используя перечисленные `vacancy_id` или `vacancy_url`. Поддерживает отчёт и файл состояния. |
...
### Отклик на вакансии из файла

Для отклика на конкретные вакансии перечислите их в CSV/TSV файле (разделитель определяется по расширению). Поддерживаемые колонки: `vacancy_id` или `vacancy_url`, `resume_id` (опционально), `message` (опционально), `enabled` (опционально).

```sh
hh-applicant-tool apply-from-file --file vacancies.csv --resume-id <RESUME_ID> --output-report report.csv --state-file state.txt
```

Если сообщение не указано в строке и не передано через `--message`, используется `reply_message` из конфига. Плейсхолдеры и варианты `{a|b}` обрабатываются так же, как в `apply-similar`. Флаг `--min-interval` по умолчанию равен 1.5 секунды.
